...
public class Mysql {
	 ...
	 public static int pingTimer = 1200;
	  ...
	  public static boolean connectMySQL()
	  {
	    ...
	    {
	      Bukkit.getConsoleSender().sendMessage(SaveStrings.pr + "§4Ein Fehler ist beim Verbindungsaufbau zur MySQL-Datenbank aufgetreten.");
	      Bukkit.getConsoleSender().sendMessage(SaveStrings.pr + e.getMessage());
	    }
	    return connected;
	  }
	  ...
	  {
	    pingTask = Bukkit.getScheduler().runTaskTimerAsynchronously(Eco.eco, new Runnable()
	    {
	      public void run()
	      {
	        try
	        {
	          if (Mysql.isConnected())
	          {
	            ...
	            s.close();
	            Bukkit.getConsoleSender().sendMessage(SaveStrings.pr + "§aDer MySQL-Server wurde erfolgreich angepingt");
	          }
	          else
	          {
	        	  Mysql.connectMySQL();
	        ...
	        {
	          Bukkit.getConsoleSender().sendMessage(SaveStrings.pr + "§4Der MySQL-Server konnte nicht angepingt werden.");
	        }
	      }
	    }, 20L, 20 * pingTimer);
	  }
	  ...
	  public static void disconect()
	  {
	    if (isConnected()) {
	      ...
	      {
	        con.close();
	        Bukkit.getConsoleSender().sendMessage(SaveStrings.pr + "§7MySQL §aerfolgreich §7Geschlossen!");
	      }
	      catch (SQLException e)
	      {
	        e.printStackTrace();
